...
parser.add_argument('--username', type=str, default="kirillkomarov", help=f"Tag to use. Defaults to today's date: kirillkomarov")
...
def build(docker_repo, tag, from_docker=None):
    ...
    docker_build_arg = f"--progress=plain --platform linux/amd64 -t {docker_container}"
